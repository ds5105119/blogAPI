...
class FollowingViewSet(mixins.CreateModelMixin,
    ...
    POST: 팔로우, request: { user: User, handle: User.handle }
    DELETE: 팔로우 또는 팔로워 삭제, request: { "user": User, "handle": User.handle, "delete_option": "following" | "follower" }
    """
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not delete_option or not handle:
            return Response({"detail": "Invalid request. 'delete' and 'handle' are required."},
